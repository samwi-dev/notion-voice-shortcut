...
| 5 | iCloud Drive 檔案被回收成 ☁️ 雲端佔位檔，編碼媒體失敗 | 已繞過 | 用「最新優先 + 限制 1」處理最新檔即可，舊檔交給下次觸發 |
| 6 | 自動化觸發策略（Wi-Fi/健身關閉/時間/充電器）尚未建立 | 主線 | 待路線 B+C 確認後建立 |
| 7 | 多筆錄音處理策略（>5 則）尚未實作 | 主線（明日處理） | 路線 B（多觸發）+ 路線 C（自我呼叫）混合方案 |
...
- **做到哪**：在「過濾檔案」加上 排序=製作日期/最新優先 + 限制=1 → **執行成功**，最新 1 筆錄音正常處理寫入 Notion
- **同時繞過**：雲端佔位檔問題（最新檔通常本機有實檔）
- **未解錯誤**：無（單筆處理 OK）
- **未決選擇**：多筆錄音（>5 則）的處理策略
  - 路線 B：保留限制 1 + 多重自動化觸發（Wi-Fi/健身/時間/充電）
  - 路線 C：保留限制 1 + 捷徑結尾自我呼叫
  - 建議 B+C 混合，明日決定
